...
from .involution_layer_cuda import Involution
import yaml
from easydict import EasyDict as edict

filename = './second/configs/tanet/tanet.yaml'
with open(filename, 'r') as f:
    cfg = edict(yaml.load(f))


class PsaInvolution(nn.Module):
    def __init__(self,
                 use_norm=True,
                 num_class=2,
                 layer_nums=[3, 5, 5],
        ...
        :param name:
        """
        super(PsaInvolution, self).__init__()
        ...
        if use_norm:
            ...
            Conv2dStaleChannels = change_default_args(bias=False)(Involution)
            Conv2dFreeChannels = change_default_args(bias=False)(nn.Conv2d)
            ConvTranspose2d = change_default_args(bias=False)(
                nn.ConvTranspose2d)
        else:
            BatchNorm2d = Empty
            Conv2dStaleChannels = change_default_args(bias=True)(Involution)
            Conv2dFreeChannels = change_default_args(bias=True)(nn.Conv2d)
        ...
        if use_bev:
            self.bev_extractor = Sequential(
                Conv2dFreeChannels(6, 32, 3, padding=1),
                BatchNorm2d(32),
                nn.ReLU(),
                # nn.MaxPool2d(2, 2),
                Conv2dFreeChannels(32, 64, 3, padding=1),
                BatchNorm2d(64),
                nn.ReLU(),
                nn.MaxPool2d(2, 2),
            )
        ...
        for i in range(layer_nums[0]):
            self.block1.add(
                Conv2dStaleChannels(num_filters[0], 3, padding=1))
        ...
        for i in range(layer_nums[1]):
            self.block2.add(
                Conv2dStaleChannels(num_filters[1], 3, padding=1))
        ...
        self.block3 = Sequential(
            nn.ZeroPad2d(1),
            Conv2dFreeChannels(num_filters[1], num_filters[2], 3, stride=layer_strides[2]),
            BatchNorm2d(num_filters[2]),
            nn.ReLU(),
        )
        for i in range(layer_nums[2]):
            self.block3.add(
                Conv2dStaleChannels(num_filters[2], 3, padding=1))
        ...
        self.RF1 = Sequential(  # 3*3
            Conv2dFreeChannels(C_Bottle*2, C, kernel_size=1, stride=1),
            BatchNorm2d(C),
            nn.ReLU(inplace=True),
            Conv2dFreeChannels(C, C_Bottle*2, kernel_size=3, stride=1, padding=1, dilation=1),
            BatchNorm2d(C_Bottle*2),
            nn.ReLU(inplace=True),
        )

        self.RF2 = Sequential(  # 5*5
            Conv2dFreeChannels(C_Bottle, C, kernel_size=3, stride=1, padding=1),
            BatchNorm2d(C),
            nn.ReLU(inplace=True),
            Conv2dFreeChannels(C, C_Bottle, kernel_size=3, stride=1, padding=1, dilation=1),
            BatchNorm2d(C_Bottle),
            nn.ReLU(inplace=True),
        )

        self.RF3 = Sequential(  # 7*7
            Conv2dFreeChannels(C_Bottle//2, C, kernel_size=3, stride=1, padding=1),
            BatchNorm2d(C),
            nn.ReLU(inplace=True),
            Conv2dFreeChannels(C, C, kernel_size=3, stride=1, padding=1),
            BatchNorm2d(C),
            nn.ReLU(inplace=True),
            Conv2dFreeChannels(C, C_Bottle//2, kernel_size=3, stride=1, padding=1),
            BatchNorm2d(C_Bottle//2),
            nn.ReLU(inplace=True),
        )
